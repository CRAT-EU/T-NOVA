...
class FlowConfigBlaster(object):
    ...
    INVURL = 'restconf/operational/network-topology:network-topology/topology/flow:1/'
    ...
    def __init__(self, host, port, ncycles, nthreads, fpr, nnodes, nflows, startflow, auth, switch, flow_mod_template=None):
        self.host = host
        self.port = port
    ...
    def get_num_nodes(self, session):
        ...
        if r.status_code == 200:
            try:
                inv = json.loads(r.content)[u'topology'][0]
                #inv = json.loads(r.content)['nodes']['node']
                nn = 0
                i = 0
                for n in inv[u'node']:
                    if re.search('openflow', n[u'node-id']) is not None:
                        nn += 1
                    i=i+1
                if nn != 0:
                    nodes = nn
...
def create_arguments_parser():
    ...
    parser.add_argument('--switch', type=int, default=1,
                           help='Switch where odl controller will forwards flows (default is s1)')
